...
export function CriterioLaplace() {
  ...
  const calcularValoresEsperados = () => {
    const nuevosValoresEsperados = resultados.map(fila => {
      const suma = fila.reduce((acc, val) => acc + val, 0);
      return suma / numEstados; // Promedio
    });

    setValoresEsperados(nuevosValoresEsperados);
    const mejor = Math.max(...nuevosValoresEsperados);
    const filaMejor = nuevosValoresEsperados.indexOf(mejor);
  ...
  return (
    <Container>
      <FormularioContainer>
                ...
                {resultados[index].map((valor, colIndex) => (
                  <td key={colIndex}>
                    <InputTabla
                      ...
                      style={{
                        fontWeight: 'normal',
                        backgroundColor: (resaltados && (index === mejorFila)) ? '#ffcccc' : 'transparent', // Resaltado para el mejor resultado
                      }}
                    />
                  </td>
                ))}
        ...
        <Boton onClick={calcularValoresEsperados}>Calcular</Boton>
      </FormularioContainer>
      ...
      <ResultadosContainer>
        {mejorResultado !== null ? (
          <>
            <Resultados>
              <h3>Resultados</h3>
              <p>La mejor alternativa es <strong>{decisiones[mejorFila]}</strong> con un valor esperado de <strong>{mejorResultado}</strong>.</p>
            </Resultados>
